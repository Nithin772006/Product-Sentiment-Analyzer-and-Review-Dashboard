...
export default function Reports() {
  ...
  const handleDownload = (productId, format) => {
    let url = "";
    if (format === "csv") {
      url = apiService.getExportCsvUrl(productId);
    } else if (format === "excel") {
      url = apiService.getExportExcelUrl(productId);
    } else if (format === "pdf") {
      url = apiService.getExportPdfUrl(productId);
    }

    if (url) {
      // Direct browser window download trigger
    ...
    <div className="max-w-7xl mx-auto px-6 py-10 w-full flex-1 flex flex-col gap-6">
      ...
      <div>
        ...
        <p className="text-sm text-[#8892b0] mt-1">
          Export scraped product reviews and computed consensus sentiment weights in tabular (CSV/Excel) or document formats (PDF).
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="card text-center py-16 flex flex-col items-center justify-center gap-3">
          ...
          <h3 className="text-lg font-semibold text-[#e8eaf6]">No tracked products matching query</h3>
            ...
            >
              ...
              <div className="flex flex-col gap-1 min-w-0 pr-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-[#8892b0] font-medium">{prod.brand || "Generic"}</span>
                </div>
                <h3 className="text-sm font-bold text-[#e8eaf6] truncate leading-tight mt-1">
                  {prod.product_name}
                </h3>
                <span className="text-[10px] text-[#8892b0] mt-0.5 block">
                  Reviews logged: <span className="font-semibold text-[#e8eaf6]">{prod.total_reviews}</span>
                </span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2.5 w-full sm:w-auto">
                <button
                  ...
                  className="btn-primary text-xs px-3.5 py-2.5 rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#1c2333] hover:bg-[#212840] flex-1 sm:flex-initial"
                >
                  <FiDownload /> CSV Data
                </button>
                <button
                ...
                <button
                  ...
                  className="btn-primary text-xs px-3.5 py-2.5 rounded-xl bg-[#3361ff] flex-1 sm:flex-initial"
                >
